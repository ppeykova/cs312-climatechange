<?php
include('OpenCage\Geocoder\AbstractGeocoder.php');
include('OpenCage\Geocoder\Geocoder.php');
require('connect.php');
$categoryType = "";
$mainArea = "";
$userAddress = "";
        ...
        for($c=0; $c < 3; $c++) {
            if(!empty($array[$j + $c])) {
                ...
                echo "<div class='col-md-3' style='border-image-width: 15em'>
                              <td class='card-body'><img class='card-img-top' style='max-height: 300px' src='data:image/jpeg;base64," . base64_encode($image) . "'/>" . "<p class='card-text'> $description </p><p class='card-text'> £ $price </p>
                              <a href=$url onclick=\"return !window.open(this.href, 'Google', 'width=800,height=800')\" target=\"_blank\"> View location map </a>
                              <button class='btn btn-primary'> Add to basket </button></td></div>";
            }
        }
        echo "</tr>";
    }
...
function geocode($url){
    $geocoder = new OpenCage\Geocoder\Geocoder('0bd6d81685ca456d862472ceadb767af');
    $result = $geocoder->geocode($url);
...
<div class="container">
    <div class="row">
        <div class="col-md-5 md-5 mx-auto" >
            <form method='post' action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                ...
                <input type="text" name="yourAddress" value = "<?php echo $userAddress ?>">
        ...
        <div class="mx-auto">
            ...
            if(!empty($userAddress)){
                $coordinates = geocode($userAddress);
                echo $coordinates;
            }
