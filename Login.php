...
if(isset($_POST['submit'])){
    ...
    if($stmt->rowCount()>0) {
        ...
        $name=$conn->prepare("SELECT user_name FROM users WHERE email =:email AND password=:password ");
        $_SESSION['name'] = $name;
        $id=$conn->prepare("SELECT id FROM users WHERE email =:email AND password=:password ");
        $_SESSION['id'] = $id;
        ?>
